...
namespace DBUtility.Dapper
{
    ...
    static partial class SqlMapper
    {
        ...
        /// <summary>
        /// This is a micro-cache; suitable when the number of terms is controllable (a few hundred,
        /// for example), and strictly append-only; you cannot change existing values. All key
        /// matches are on **REFERENCE** equality. The type is fully thread-safe.
        /// </summary>
        partial class Link<T, TValue> where T : class
        {
            public static bool TryGet(Link<T, TValue> link, T key, out TValue value)
            {
                while (link != null)
                {
            ...
            public static bool TryAdd(ref Link<T, TValue> head, T key, ref TValue value)
            {
                bool tryAgain;
                do
                {
                    ...
                    var newNode = new Link<T, TValue>(key, value, snapshot);
            ...
            private Link(T key, TValue value, Link<T, TValue> tail)
            {
                Key = key;
                Value = value;
                Tail = tail;
            ...
            public Link<T, TValue> Tail { get; private set; }
        }
        ...
        /// </summary>
        /// <returns>
        /// A sequence of data of the supplied type; if a basic type (int, string, etc) is queried
        /// then the data from the first column in assumed, otherwise an instance is created per row,
...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
        ...
        /// </summary>
        /// <remarks>
        /// the dynamic param may seem a bit odd, but this works around a major usability issue in
        /// vs, if it is Object vs completion gets annoying. Eg type new [space] get new object
        /// </remarks>
        /// <returns>
        /// A sequence of data of the supplied type; if a basic type (int, string, etc) is queried
        /// then the data from the first column in assumed, otherwise an instance is created per row,
...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
...
)
        {
            ...
            try
            {
                ...
                // still need something in the "finally" to ensure that broken SQL still results in
                // the connection closing itself
                return result;
            }
            catch
            {
        ...
        private static IEnumerable<T> QueryInternal<T>(this IDbConnection cnn, string sql, object param, IDbTransaction transaction, int? commandTimeout, CommandType? commandType)
        {
            ...
            try
            {
                ...
                // happy path; close the reader cleanly - no need for "Cancel" etc
                reader.Dispose();
                reader = null;
            }
            finally
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, string splitOn = "Id", int? commandTimeout = null, CommandType? commandType = null
            ...
            , IDictionary<string, object>
        {
            ...
            private object SetValue(string key, object value, bool isAdd)
            {
                ...
                if (oldLength <= index)
                {
                    // we'll assume they're doing lots of things, and grow it to the full width of
                    // the table
        ...
        {
            // initially we tried TVP, however it performs quite poorly. keep in mind SQL support up
            // to 2000 params easily in sp_executesql, needing more is rare
        ...
        public static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity, bool checkForDuplicates, bool removeUnused)
        {
            ...
            foreach (var prop in props)
            {
                ...
                {
                    // stack is now [parameters] [parameters] [parameter] blindly add
        ...
        {
            // no point using special per-type handling here; it boils down to the same, plus not all
            // are supported anyway (see: SqlDataReader.GetChar - not supported!)
...
Type type, IDataReader reader, int startBound = 0, int length = -1, bool returnNullIfFirstMissing = false
        ...
        /// <summary>
        /// How should connection strings be compared for equivalence? Defaults to
        /// StringComparer.Ordinal. Providing a custom implementation can be useful for allowing
        /// multi-tenancy databases with identical schema to share startegies. Note that usual
        /// equivalence rules apply: any equivalent connection strings <b>MUST</b> yield the same hash-code.
        /// </summary>
...
string name, object value = null, DbType? dbType = null, ParameterDirection? direction = null, int? size = null
        ...
        /// <summary>
        /// If true, the command-text is inspected and only values that are clearly used are included
        /// on the connection
        /// </summary>
...
#pragma warning restore 612, 618
    ...
    /// <summary>
    /// Represents simple memeber map for one of target parameter or property or field to source
    /// DataReader column
    /// </summary>
    ...
    /// <summary>
    /// Implements custom property mapping by user provided criteria (usually presence of some custom
    /// attribute with column to member mapping)
    /// </summary>
    ...
    // Define DAPPER_MAKE_PRIVATE if you reference Dapper by source and you like to make the Dapper
    // types private (in order to avoid conflicts with other projects that also reference Dapper by source)
